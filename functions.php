...
function novus_contact_info() {
	$infos = rwmb_meta( 'contact_info', ['object_type' => 'setting'], 'novus-settings' );

	$infos_html = '<div class="info">';

	foreach( $infos as $i ) :
		$infos_html += '<a href="' . $i['url'] . '"><img width="40" height="40" src="' . wp_get_attachment_url( $i['icon'] ) . '">'. $i['text'] . '</a>';
	endforeach;

	return $infos_html += '</div>';
}

add_shortcode( 'fl_contact', 'novus_contact_info' );